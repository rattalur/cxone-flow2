...
When configuring the \texttt{base-url} for the Azure DevOps Enterprise connection configuration,
as described in Section \ref{sec:connection-element}, the \texttt{base-url} should include
the collection.  Some organizations only have the collection named \texttt{DefaultCollection},
but it is possible to have multiple different collections. 

If your organization's Azure DevOps Enterprise system has a single collection containing 
multiple projects, a single Azure DevOps Enterprise connection can be used for all projects.
A single connection configuration assumes that the user account configured with 
the \texttt{api-auth} and/or \texttt{clone-auth} elements has been granted committer
access to each project in the collection.

If your organization's Azure DevOps Enterprise system has multiple collections that will
use \cxoneflow to orchestrate scans, a separate connection configuration will be required
for each collection.  The \texttt{repo-match} configuration element, described in
Section \ref{sec:scm-block-element} should also include the collection name as part of the
match criteria.
